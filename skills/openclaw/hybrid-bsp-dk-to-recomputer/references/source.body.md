...
- [从 Jetson 开发环境创建自定义 BSP 包](https://wiki.seeedstudio.com/cn/make_diy_bsp_for_jetson/)
- [将 Orin Nano DK 的 /home 迁到 reComputer](https://wiki.seeedstudio.com/cn/migrate_home_data_from_jetson_orin_nano_developer_kit_to_recomputer/)
- [刷写 JetPack 到指定产品](https://wiki.seeedstudio.com/cn/flash/jetpack_to_selected_product/)
...
进入恢复模式的说明见：[刷写 JetPack 到指定产品](https://wiki.seeedstudio.com/cn/flash/jetpack_to_selected_product/)
...
```
fb1d502d9e869d67226eaf71bbe2462fab4e2f1dacf8a6a7fa59057c66a2e845
```
...
例如：`NVIDIA Jetson Orin Nano Engineering Reference Developer Kit Super`

原因：目标板 conf 对 SKU 0005 选用的是 NVIDIA 的 `tegra234-p3768-0000+p3767-0005-nv-super.dtb`，不会换成 `tegra234-j401-*-recomputer.dtb`，因此 model 字符串仍像官方 DevKit。不能单凭这行判断「刷错成 DevKit 包」。
...
Hybrid 刷完后，`/boot/*.dtbo` 里经常仍是 DevKit 备份带来的通用 overlay 列表，不一定出现 `tegra234-dcb-p3767-0000-hdmi.dtbo` 或 Seeed 摄像头 overlay。Seeed HDMI/摄像头相关配置更多在目标板新生成的 QSPI / UEFI overlay 路径上生效。
...
| 有线网 | Classic: `enP8p1s0` 等为 `UP`；Super: 见技术说明 C |
| Wi-Fi | `wlP1p1s0` 为 `UP` |
| 显示 | 桌面可用；或 `xrandr` 有输出 |
| 用户环境 | 原 DevKit 的用户、软件、数据仍在 |
| CUDA | `nvcc --version` 能跑通（本例 12.6），说明 APP 克隆完整 |

> **NOTE:** 接口名如 `enP8p1s0` 和 `wlP1p1s0` 取决于内核/PCIe 总线，不同板子或内核版本可能不同。用 `ip -br link` 查看实际名称——按类型找以太网和 Wi-Fi 接口，不要按精确名称匹配。

- Classic: 重点验证 dual camera 配置（`imx219-dual-seeed`）
- Super: 重点验证 quad camera 配置（`imx219-quad-seeed`）。`/boot` 中没有显示 `imx219-quad-seeed.dtbo` 文件名也不能单独判定失败，因为 overlay 可能已由 QSPI/UEFI 路径应用。

> **Super 实测结果（来自 Seeed Wiki）：**
...
2. 从备份抽出 `/home`（`nvme0n1p1.tar.zst`），或按 [将 Orin Nano DK 的 /home 迁到 reComputer](https://wiki.seeedstudio.com/cn/migrate_home_data_from_jetson_orin_nano_developer_kit_to_recomputer/) 操作。
...
A: 那是源机与目标同为 Seeed 板时的示例。源机是官方 DevKit 时，备份必须先用 `jetson-orin-nano-devkit-nvme`，再按本教程 Hybrid 适配目标板。
...
本例克隆的 DevKit RT 内核在 Super 的 LAN7430 上加载 `lan743x` 时发生 kernel Oops。最终 Hybrid BSP 默认禁用 `lan743x`（见技术说明 A 第 3 条），因此板载有线网暂不可用，Wi-Fi 不受影响。
