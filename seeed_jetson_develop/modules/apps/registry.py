...
_DA3_RUN_CMDS = [
    ...
    "bash -lc 'set -e; echo \"[3/5] start tutorial container + run GUI preview in background xauth mode\"; DISP=${DISPLAY:-}; if [ -z \"$DISP\" ]; then if [ -S /tmp/.X11-unix/X1 ]; then DISP=:1; else DISP=:0; fi; fi; XAUTH=/run/user/1000/gdm/Xauthority; if [ ! -f \"$XAUTH\" ]; then XAUTH=/home/seeed/.Xauthority; fi; pick_cam(){ for d in /dev/video0 /dev/video3 /dev/video1 /dev/video2; do [ -e \"$d\" ] || continue; if timeout 3s v4l2-ctl -d \"$d\" --stream-mmap --stream-count=1 --stream-to=/tmp/cam_probe.raw >/dev/null 2>&1; then echo ${d#/dev/video}; return 0; fi; done; echo 0; }; CAM=$(pick_cam); echo \"[info] start container with DISPLAY=$DISP XAUTH=$XAUTH CAMERA_ID=$CAM\"; sudo docker rm -f depth_anything_v3 >/dev/null 2>&1 || true; sudo docker run -d --name depth_anything_v3 --restart unless-stopped --runtime=nvidia --network host --ipc host --privileged -e DISPLAY=$DISP -e XAUTHORITY=$XAUTH -e CAMERA_ID=$CAM -e QT_X11_NO_MITSHM=1 -e LIBGL_ALWAYS_SOFTWARE=1 -v /tmp/.X11-unix:/tmp/.X11-unix -v $XAUTH:$XAUTH:ro -v /dev:/dev chenduola6/depth_anything_v3:jp6.2 bash -lc \"set -e; cd /workspace/ros2-depth-anything-v3-trt; source install/setup.bash; if ls onnx/*.engine >/dev/null 2>&1; then printf \\\"[skip] engine already exists\\\\n\\\"; else printf \\\"[info] first run building TensorRT engine, this can take several minutes\\\\n\\\"; ros2 run depth_anything_v3 generate_engines onnx; fi; exec ./run_camera_depth.sh\"; sudo docker ps -a --filter name=depth_anything_v3'",
